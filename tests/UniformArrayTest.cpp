#include "UniformArrayTest.h"
#include "nuanceur/Builder.h"

void CUniformArrayTest::Run()
{
	using namespace Nuanceur;

	auto b = CShaderBuilder();

	{
		auto uniformFloatArray = CArrayFloat4Value(b.CreateUniformArrayFloat4("floatValues", 4));
		auto outputColor = CFloat4Lvalue(b.CreateOutput(Nuanceur::SEMANTIC_SYSTEM_COLOR));

		outputColor = Load(uniformFloatArray, NewInt(b, 2));
	}

	auto result = CVector4(55.f / 255.f, 244.f / 255.f, 255.f / 255.f, 0);

	CTestContext testContext;
	for(int i = 0; i < 4; i++)
	{
		testContext.uniformBuffer.push_back({0, 0, 0, 0});
	}
	testContext.uniformBuffer[2] = result;
	testContext.expectedValue = result;

	Submit(b, testContext);
}
